...
def convolve(image, weight):
    ...
    :param image: 图片中每个RGB分量
    :param weight: 权值，此处为3x3的矩阵
    :return:
    """
    print 'image shape:', image.shape, ' weight:', weight
    ...
    for i in range(height_new):
        for j in range(width_new):
            # 因为卷积操作实际上就是滑动的3x3的移动窗口与权值矩阵做元素乘法，
            # 然后将各个乘法结果的相加一起成为卷积的结果；这样，一个3x3的移动窗口计算结束；从左往右 移动窗口，
            # 选择新的3x3元素矩阵，完成相同的操作. 目前的窗口滑动stride为1,即每次窗口变动间隔为1个元素;
            image_new[i, j] = np.sum(image[i:i + h, j:j + w] * weight)

    image_new = image_new.clip(0, 255)  # 限制颜色分量的值的范围为0~255
...
if __name__ == "__main__":
    ...
    a = np.array(A)
    soble_x = np.array(([-1, 0, 1], [-2, 0, 2], [-1, 0, 1]))  # 卷积核
