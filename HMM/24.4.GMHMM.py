...
if __name__ == "__main__":
    # 高斯模型隐马尔可夫模型 (即发射矩阵已经不是离散的，而是连续的)
    ...
    for i in range(n):
        ...
        covars[i] = np.diag(np.random.rand(2)+0.1)*100    # np.random.rand ∈[0,1)
    print '方差：\n', covars
    ...
    # 估计参数
    model = hmm.GaussianHMM(n_components=n, covariance_type='full', n_iter=5)
    model = model.fit(sample)

    print '========='
    print np.asarray(covars)
    for covar in covars:
        print linalg.eigvalsh(covar) <= 0
        print '-> ', np.allclose(covar, covar.T)
    print model.covars_

    ar = np.array([[15.77761362,  15.77761362], [15.77761362,  15.77761362]])
    print linalg.eigh(ar)
    print linalg.eigvalsh(ar)
