...
if __name__ == '__main__':
    # DPGMM 高斯混合模型(贝叶斯高斯混合模型: 即认为参数theta也是服从某一种分布(dirichlet 分布)的，而不是固定值。)
    np.random.seed(0)
    cov1 = np.diag((1, 2))
    N1 = 500
    N2 = 300
    ...
    gmm.fit(x)
    centers = gmm.means_    # 均值
    covs = gmm.covariances_    # 方差
    print 'GMM均值 = \n', centers
    print 'GMM方差 = \n', covs
    y_hat = gmm.predict(x)
    ...
    clrs = list('rgbmy')
    for i, cc in enumerate(zip(centers, covs)):     # 以分类样本的均值和协方差矩阵的特征值和特征向量分别作为椭圆的圆心，宽和高。
        center, cov = cc
        value, vector = sp.linalg.eigh(cov)    # 计算矩阵的特征值和特征向量
        print '特征值:', value
        print '特征向量:', vector
