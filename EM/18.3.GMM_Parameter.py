...
if __name__ == '__main__':
    ...
    # spherical:协方差矩阵是对角单位矩阵，此时高斯模型在二维平面映射为圆形
    # diag: 协方差矩阵是对角矩阵，但是对角线的元素不相同，此时高斯模型在二维平面映射为椭圆
    # tied: GMM各自的协方差矩阵相同
    # full: GMM各自的协方差矩阵为一般矩阵;此时映射到二维平面，实际与二维平面有夹角。
    ...
    for i, type in enumerate(types):
        ...
        bic[i] = gmm.bic(x)
    print '错误率：', err.ravel()
    print 'BIC：', bic.ravel()   # BIC: 评价模型的好坏；越低越好
    xpos = np.arange(4)
    ax = plt.axes()     # 多柱状图
    b1 = ax.bar(xpos-0.3, err, width=0.3, color='#77E0A0')      # 多Y轴
    b2 = ax.twinx().bar(xpos, bic, width=0.3, color='#FF8080')  # 多Y轴
    plt.grid(True)
    bic_min, bic_max = expand(bic.min(), bic.max())
    plt.ylim((bic_min, bic_max))
    plt.xticks(xpos, types)     # 设置X轴
