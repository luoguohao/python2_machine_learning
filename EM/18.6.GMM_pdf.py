...
if __name__ == '__main__':
    ...
    print gmm.score_samples(grid_test)  # 对数似然函数值
    grid_hat = -gmm.score_samples(grid_test)
    grid_hat = grid_hat.reshape(x1.shape)
    plt.figure(figsize=(9, 7), facecolor='w')
    ax = plt.subplot(111)
    cmesh = plt.pcolormesh(x1, x2, grid_hat, cmap=plt.cm.Spectral)
    plt.colorbar(cmesh, shrink=0.8)    # 色调盘
    CS = plt.contour(x1, x2, grid_hat, levels=np.logspace(0, 2, num=levels, base=10), colors='w', linewidths=1)    # 等值线
    plt.clabel(CS, fontsize=9, inline=1, fmt='%.1f')    # 等值线上的值
    ...
    plt.title(u'GMM对数似然函数值', fontsize=20)
    plt.grid(True)
    plt.show()
