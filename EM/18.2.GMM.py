...
if __name__ == '__main__':
    ...
    change = (gmm.means_[0][0] > gmm.means_[1][0])  # 判断预测分类是否顺序一致
    if change:
        z = y_hat == 0
        y_hat[z] = 1
        y_hat[~z] = 0
    ...
    plt.pcolormesh(x1, x2, grid_hat, cmap=cm_light)    # 分类颜色网格
    plt.scatter(x[:, 0], x[:, 1], s=50, c=y.reshape(-1), marker='o', cmap=cm_dark, edgecolors='k')  # 训练集
    plt.scatter(x_test[:, 0], x_test[:, 1], s=60, c=y_test.reshape(-1), marker='^', cmap=cm_dark, edgecolors='k')   # 测试集

    p = gmm.predict_proba(grid_test)
    p = p[:, 0].reshape(x1.shape)
    CS = plt.contour(x1, x2, p, levels=(0.2, 0.5, 0.8), colors=list('rgb'), linewidths=2)   # 等值线
    plt.clabel(CS, fontsize=15, fmt='%.1f', inline=True)    # 等值线上增加label显示
    ax1_min, ax1_max, ax2_min, ax2_max = plt.axis()
    xx = 0.9*ax1_min + 0.1*ax1_max  # 计算文字显示位置
    yy = 0.1*ax2_min + 0.9*ax2_max
    plt.text(xx, yy, acc_str, fontsize=18)  # 添加文字描述
