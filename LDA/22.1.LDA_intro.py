...
if __name__ == '__main__':
    ...
    stop_list = set('for a of the and to in'.split())   # 停止词
    ...
    dictionary = corpora.Dictionary(texts)  # 语料词典
    V = len(dictionary)

    corpus = [dictionary.doc2bow(text) for text in texts]   # 转成语料索引
    model = models.TfidfModel(corpus)
    print corpus
    corpus_tfidf = model[corpus]    # 转为TD-IDF
    ...
    hda = models.HdpModel(corpus_tfidf, id2word=dictionary)     # 结构化LDA模型
